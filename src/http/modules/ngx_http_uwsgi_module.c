...
static ngx_command_t ngx_http_uwsgi_commands[] = {
    ...
    { ngx_string("uwsgi_ssl_certificate_key"),
     ...
     { ngx_string("uwsgi_ssl_enc_certificate"),   //add by TASS gujq for GM
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_str_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_uwsgi_loc_conf_t, certificates_enc),
      NULL },

    { ngx_string("uwsgi_ssl_enc_certificate_key"),   //add by TASS gujq for GM
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_str_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_uwsgi_loc_conf_t, certificate_enc_keys),
      NULL },
...
ngx_http_uwsgi_set_ssl(ngx_conf_t *cf, ngx_http_uwsgi_loc_conf_t *uwcf)
{
    ...
    if (uwcf->ssl_certificate.len) {
        ...
        if (ngx_ssl_certificate(cf, uwcf->upstream.ssl, &uwcf->ssl_certificate,
                                &uwcf->ssl_certificate_key, &uwcf->certificates_enc, &uwcf->certificate_enc_keys, uwcf->ssl_passwords)
            != NGX_OK)
        {
            return NGX_ERROR;
        }
