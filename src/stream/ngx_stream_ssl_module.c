...
static ngx_command_t  ngx_stream_ssl_commands[] = {
    ...
    { ngx_string("ssl_enc_certificate"),      //add by TASS gujq for GM
      NGX_STREAM_MAIN_CONF|NGX_STREAM_SRV_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_str_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_ssl_conf_t, certificates_enc),
      NULL },

    { ngx_string("ssl_enc_certificate_key"),  //add by TASS gujq for GM
      NGX_STREAM_MAIN_CONF|NGX_STREAM_SRV_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_str_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_ssl_conf_t, certificate_enc_keys),
      NULL },
...
ngx_stream_ssl_merge_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (conf->certificate_values) {
        ...
        if (ngx_ssl_certificates(cf, &conf->ssl, conf->certificates,
                             conf->certificate_keys, &conf->certificates_enc, &conf->certificate_enc_keys, conf->passwords) //add by TASS gujq for GM
            != NGX_OK)
        {
            return NGX_CONF_ERROR;
        }
