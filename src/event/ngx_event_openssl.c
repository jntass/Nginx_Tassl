...
ngx_ssl_certificates(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_array_t *certs,
    ngx_array_t *keys, ngx_str_t *enc_certs, ngx_str_t *enc_keys, ngx_array_t *passwords)
{
    ngx_str_t   *cert, *key;
    ngx_uint_t   i;
    ...
    for (i = 0; i < certs->nelts; i++) {
        ...
        if (ngx_ssl_certificate(cf, ssl, &cert[i], &key[i], enc_certs, enc_keys, passwords)
            != NGX_OK)
        {
            return NGX_ERROR;
        }
...
ngx_ssl_certificate(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_str_t *cert,
    ngx_str_t *key, ngx_str_t *enc_cert, ngx_str_t *enc_key, ngx_array_t *passwords)
{
    char            *err;
    X509            *x509;
    EVP_PKEY        *pkey;
    ...
    if(enc_cert->len > 0){
    	...
    	pkey = ngx_ssl_load_certificate_key(cf->pool, &err, enc_key, passwords);
    	if (pkey == NULL) {
    	    if (err != NULL) {
    	        ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0,
    	                      "cannot load certificate key \"%s\": %s",
